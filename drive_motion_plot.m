function drive_motion_plot()
    ...
    % Solving the ODE.
    for j = 1:2:7
        options = odeset('RelTol',1*10^(j*-1),'AbsTol',1e-10);
%         options = odeset('RelTol',1e-8,'AbsTol',1*10^(j*-1));
        ...
        hold on
        plot(t,theta(:,1));
        xlabel('t'); ylabel('Theta'); title('Theta vs t (A = 0.5)'); 
        legend(sprintf('RelTol = 1e-%d',j));
        hold off
    end
...
%         options = odeset('RelTol',1e-8,'AbsTol',1*10^(j*-1));
        ...
        % theta as function of time
        plot(t,theta(:,1));
        xlabel('t'); ylabel('Theta'); title('Theta vs t (A = 1.2)');
    end
